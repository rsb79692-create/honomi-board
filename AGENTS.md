...
- **利用者を外す操作は「削除」だけ**（2026-09-05 に変更。一時的に止める `active=false` の操作は撤去した）。
  ...
  ⚠ **共有（`shares` / `shareKeys`）は消さない。** ボードに紐づくもので利用者には紐づかない。
...
- `active` はルール条文（`members` / `config` / `field` / `shares` / `shareKeys` / `rooms`）が
  今も見ているので**フィールドごと消してはならない**。招待時に `true` を書くだけで、
  `false` を作る経路はもう無い（`members/$uid` の `.validate` も `role` と `active` を必須にしている）。
  古いデータに `false` が残っている場合に備え、表示と `newRoomHtml` の分岐は残してある。
...
- **共有リンクの確認（未認証で実測する）**: `?auth=` を付けずに REST を叩き、
  ...
  ④ 「共有をやめる」直後に `shareKeys/{token}` が 401 になること、を確かめる。
  さらに**共有相手の idToken**で `rooms/{rid}/board/owner/vision` へ PUT して 401 になること。
...
- ⚠⚠ **失効させる操作は「共有をやめる」「作り直す」「ボードを削除する」の3つだけ。**
  **利用者（`members`）を削除しても、部屋のメンバーから外しても、共有は止まらない**
  （ボードに紐づき、相手は `members` に載らないため）。
...
- ⚠ **相手は同じボードの他の相手の列も読める**（`rooms/$rid` を丸ごと読むため）。
  1ボード1相手の運用なら起きない。複数人へ共有するときは踏まえること
- ⚠ **`board/guest/$sid` に大きさの上限が無い。** `.validate` は欄の名前（4種）しか縛らないので、
  相手は任意の大きさ・形のデータを自分の列へ書ける
...
- ⚠ **正式メンバー（mgr）は経営ボードに自分の列を持たない。** 読むことはできる
  ...
  現場責任者に書いてもらうときは、メンバー追加ではなく**共有リンクを渡す**
...
- ⚠⚠ **`honomi` ブロックが `auth != null` しか要求していない。** そのため
  **Firebase Auth のアカウントを持つ人は誰でも timecard の勤怠データを読み書きできる**。
  メール／パスワードのサインアップは（招待フローが使うため）無効化できないので、
  これは共有相手のアカウントにも当てはまる。
  ⚠ ただし**この穴は今回の変更で作ったものではない**。以前から、
