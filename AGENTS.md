...
並び順は **ビジョン → タスク → アウトプット → 相談**。双方がまったく同じURL・同じ中身を見る。
**見る人によって左右が入れ替わることはない**（2026-09-05 の「自分側／相手側」方式は廃止）。
どの欄を誰が書くかは**欄で決まっている**ので、振り分けに `by` は使わない。

| 画面の名前 | DB キー | 書ける人 | 役割 |
|---|---|---|---|
| ビジョン | `ceoMind` | 社長のみ | **読む場所**。1行に畳んで出す（後述） |
| タスク | `toManager` | 社長のみ | **依頼を受ける場所**。やってほしいこと・確認してほしいこと |
| アウトプット | `toCeo` | その部屋の現場側 | **結果を返す場所**。やったこと・対応結果。**投稿者名と日時を出す** |
| 相談 | `mgrAsk` | その部屋の現場側 | 判断してほしいこと |

⚠⚠ **DB のキーは旧名のまま。表示名だけを変えている**（2026-09-05）。
`toManager` は旧「指針」、`toCeo` は旧「報告」。
**キーを改名してはならない。** 改名すると本番の既存データが読めなくなる。
役割を混ぜないこと（ビジョン＝読む／タスク＝依頼／アウトプット＝結果）。
...
- 見せるのは**その人がいる部屋のビジョンとタスクの本文だけ**（見出しもこの2語で固定）。
  UID・メールアドレス・他の人・その人がいない部屋・アウトプット（`toCeo`）・相談（`mgrAsk`）・
  投稿者名・投稿日時・`field` は入れない。
  ⚠ これは**意図的な線引き**であって、画面に出していないからではない（4欄とも画面には出る）。
  `viewPayload()` の許可リスト（`VIEW_KEYS`）を画面側の `cards()` と共通化してはならない
...
- **写しを書くのは社長のブラウザだけ。** ただし閲覧リンクへ載せるのはビジョンとタスクだけで、
  この2欄はルール上まさに社長しか書けない。**現場側が書いた内容は写しに載らない**ので実害は無い

- ⚠ **アウトプットの「投稿者名・日時」は本人申告。** `by` / `createdAt` はクライアントが
  自分で入れる値で、ルール側に `.validate` が無いため詐称できる。
  `board` はリストごと置き換える方式なので、`by === auth.uid` を `.validate` で強制すると
  **他人の行を含むリストが書けなくなる**（並べ替えも落ちる）。強制するなら保存方式から設計し直すこと。
  現状は `posterName()` が**その部屋にいない UID を「だれか」へ丸める**ところまで。
  ⚠ **この表示を「誰がいつ報告したかの記録」として業務判断に使わないこと。**
...
- ⚠ **同じ欄を同時に書くと後勝ちになる。** `saveBoard` / `saveField` は配列を丸ごと置換する。
  同じ部屋に現場側が2人いる場合、アウトプット（`toCeo`）を同時に書くと片方の行が消えうる
